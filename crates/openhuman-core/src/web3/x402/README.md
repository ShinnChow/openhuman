...
| `ops/client.rs` / `ops/errors.rs` | `X402Client`/`try_paid_request`, `X402PaymentResult`, `X402Error`, `handle_402`/`handle_402_and_pay`, `wallet_signer` (Solana signer resolution through the wallet module), `build_solana_payment` (also in `solana_payment.rs`) and `build_evm_payment`. |
