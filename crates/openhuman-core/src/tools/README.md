...
| `crates/openhuman-core/src/tools/schemas.rs` (`include!`s `schemas_part_01.rs`, `schemas_part_02.rs`) | JSON-RPC `tools` namespace controllers + `handle_*` fns. `all_controller_schemas` / `all_registered_controllers` (re-exported as `all_tools_*`). |
...
- **RPC surface is intentionally tiny** (6 methods) — anything not in `schemas_part_01.rs`/`schemas_part_02.rs` is agent-only.
