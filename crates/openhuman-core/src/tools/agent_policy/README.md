...
- `crates/openhuman-core/src/agent/harness/session/runtime.rs` (via its `runtime_impl_01_part_01.rs` include) — `rebuild_tool_policy_session` re-runs `build_session_from_refs` after `hide_tools`.
...
- `crates/openhuman-core/src/agent/tinyagents/middleware_part_07.rs` — `ToolPolicyMiddleware` is the runtime enforcement point: `is_denied()` for direct calls, `blocks_execution()` for the `use_skill` route. The snapshot reaches it through `ToolPolicyEnforcement` in `agent/tinyagents/mod_part_01.rs`.
