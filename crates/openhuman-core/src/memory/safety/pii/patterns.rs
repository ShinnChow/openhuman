//! Per-identifier PII regexes, candidate-gated match collection, and the
//! `redact_pii`/`has_likely_pii`/`has_likely_email` public entry points.
//!
//! Split out of `pii.rs`. Normalization and the byte-oriented candidate
//! pre-filter live in [`super::normalize`]; checksum/structural validators
//! (Luhn, IBAN mod-97, Verhoeff, ...) live in [`super::checksums`].
