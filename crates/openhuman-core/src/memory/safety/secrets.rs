//! Generic secret/credential scrubbing plus the sanitized-value types shared
//! with [`super::pii`].
//!
//! Split out of the original `safety.rs` (see that module's doc comment for
//! why this policy lives in OpenHuman rather than the memory engine). This
//! half owns the block/redaction pattern lists, the sensitive-key
//! classifier, the JSON depth cap, and the `sanitize_text`/`sanitize_json`
//! entry points; PII-specific regexes and checksum validators live in
//! [`super::pii`].
