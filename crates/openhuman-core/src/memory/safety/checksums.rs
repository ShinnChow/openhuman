//! Checksum and structural validators for PII candidates: Luhn (credit
//! cards), IBAN mod-97, Verhoeff (Aadhaar), the CPF/CNPJ/CUIT digit
//! algorithms, and the Spanish DNI/NIE check letters.
//!
//! Split out of `pii.rs`. Consumed by the candidate matching in
//! [`super::pii_patterns`], which gates a formatted-looking match on the
//! matching validator before treating it as a confirmed hit.
