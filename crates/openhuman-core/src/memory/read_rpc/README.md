...
at the `memory/` root drives cross-file behavior with `use super::*;` against
this module, including a `#[cfg(test)]`-only `with_connection` door for
asserting on the rows a handler wrote directly.
