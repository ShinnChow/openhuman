//! `RecordingProvider`'s `Core`, `Recall`, `Portability`, `Ingest`,
//! `Documents`, `Tree`, `Entities`, `Graph`, `Diff`, `Goals`, and
//! `ToolMemory` family implementations — the write/tree/graph half.
//!
//! Split out of the original `test_support.rs`; see [`super::fixtures`] for
//! `RecordingProvider`'s type and constructors.
...
impl MemoryTree for RecordingProvider {
    ...
    async fn summarise(
        &self,
        inputs: &[crate::memory::api::provider::content::SummaryInput],
        _context: &crate::memory::api::provider::content::SummaryContext,
    ) -> Result<crate::memory::api::provider::content::SummaryOutput, MemoryError> {
        self.record(Call {
    ...
    async fn runtime_read_node(
        ...
        _namespace: &str,
        _node_id: &str,
    ) -> Result<Option<crate::memory::api::tree::TreeNode>, MemoryError> {
    ...
    async fn runtime_read_children(
        ...
        _namespace: &str,
        _parent_id: &str,
    ) -> Result<Vec<crate::memory::api::tree::TreeNode>, MemoryError> {
