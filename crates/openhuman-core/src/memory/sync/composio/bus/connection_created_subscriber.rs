//! Routes `ComposioConnectionCreated` events: eagerly warms the
//! integrations cache once the connection is confirmed active, then runs
//! the optional provider bootstrap + initial sync and auto-registers the
//! connection as a memory source.
...
pub(super) async fn toolkit_is_memory_source_registrable(
    ...
    let Ok(binding) = crate::memory::binding::for_config(config) else {
        tracing::warn!(
            toolkit = %toolkit,
    ...
    async fn handle(&self, event: &DomainEvent) {
        ...
        tokio::spawn(async move {
            ...
            let (src_max_items, src_sync_depth_days) = {
                let registry_sources = crate::memory::sources::list_sources()
                    .await
                    .unwrap_or_default();
                registry_sources
                    .iter()
                    .find(|s| {
                        s.kind == crate::memory::sources::SourceKind::Composio
            ...
            match wait_for_connection_active(&backend_client, &connection_id).await {
                Ok(status) => {
                    ...
                    // credentials since.
                    let live_config = match crate::config::rpc::reload_config_from_paths(
                        &config.config_path,
                        &config.workspace_dir,
                    )
                    .await
            ...
            if !config.onboarding_completed {
                ...
                // already performs.
                if let Err(e) = crate::integrations::composio::ops::composio_get_user_profile(
                    &config,
                    &connection_id,
                )
                .await
                ...
                let host_config = match crate::config::rpc::reload_config_from_paths(
                    &config.config_path,
                    &config.workspace_dir,
                )
                .await
                ...
                // domain uses now.
                let attempted = crate::integrations::composio::ops::run_sync_within_budget(
                    &host_config,
                    &toolkit,
                    &connection_id,
                    "connection_created",
                ...
                match attempted {
                    Ok(pass) => {
                        ...
                        crate::integrations::composio::periodic::record_sync_success(
                            &toolkit,
                            &connection_id,
                        );
                    }
