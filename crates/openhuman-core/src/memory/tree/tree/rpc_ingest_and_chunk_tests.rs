...
async fn backfill_status_reports_the_drivers_pending_count() {
    use crate::memory::api::provider::types::QueueStats;
...
async fn backfill_status_reports_the_drivers_flag_when_the_counts_are_empty() {
    ...
    let driver = std::sync::Arc::new(
        crate::memory::binding::FixedDiagnostics::new(Default::default(), QueueStats::default())
            .backfilling(),
    );
    crate::memory::binding::install_for_test(
        &cfg.workspace_dir,
        &cfg.subsystems.memory,
        driver as std::sync::Arc<dyn crate::memory::api::provider::MemoryProvider>,
    );
