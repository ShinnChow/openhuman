//! Persisting the app-session auth profile: secret encryption helpers and
//! the `store_session` / `store_session_with_deferred_validation` entry
//! points and their shared worker.
...
use crate::security::credentials::session_support::{
    is_local_session_token, local_session_user_id, summarize_auth_profile, LOCAL_SESSION_USER_ID,
};
...
use super::login_services::{is_embedder_host, start_login_gated_services};
use super::session_validation::{
    auth_me_store_failure_is_transient, fallback_session_user_for_deferred_validation,
    fetch_current_user_for_session_store, jwt_exp_live_at, normalize_local_session_user,
...
async fn store_session_inner(
    ...
    if !local_session {
        match decode_jwt_exp(trimmed_token) {
            Some(exp) => {
                metadata.insert(
                    crate::security::credentials::session_support::SESSION_EXPIRES_AT_META
                        .to_string(),
                    exp.to_rfc3339(),
                );
                tracing::info!(
    ...
    if let Err(error) = crate::cron::seed::prune_retired_jobs(&effective_config) {
        tracing::warn!(
            error = %error,
