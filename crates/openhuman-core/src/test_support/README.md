...
- `crate::memory::read_rpc` — `wipe_all_rpc` to clear memory-tree rows, content dirs, and sync state.
...
- **CI runs only `introspect` tests**: the `rpc_tests.rs` wipe test needs a memory driver that serves `Maintenance` (`memory-engine-seams`, off in the neither-set CI profile), so the CI Lite lane filters to `test_support::introspect::` and the `rpc` tests only run in a default-features `cargo test --features e2e-test-support`.
