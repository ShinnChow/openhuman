...
- `store.rs` adapts the historical `store::*(workspace_dir, …)` call surface
  (RPC ops, the harness turn loop, the heartbeat continuation runtime, the
  agent tools, and post-turn accounting) onto the crate's `graph.goals` KV
  store, converting the crate's `ThreadGoal`/error types to the local ones.
- `schemas.rs` exposes `openhuman.thread_goals_<function>` JSON-RPC: `get`,
  `set`, `complete`, `pause`, `resume`, `clear`.
...
- `continuation.rs` is the heartbeat adapter that injects one autonomous
  ...
  process-wide `Semaphore(1)`.
- `runtime.rs` folds completed-turn usage into the active goal
...
Callers: `agent::harness::session::turn`, `agent::orchestration::tools`
(context preparation), `tools::ops`, and controller wiring in `core::all` and
`core::runtime::services`. The external `thread_goals.*` RPC namespace and
`goal_*` tool names remain stable.
