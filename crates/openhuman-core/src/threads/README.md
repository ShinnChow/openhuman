...
| `crates/openhuman-core/src/threads/ops.rs` | Thin shell: `include!`s `ops_part_01.rs` (thread + message ops, title generation, delete) and `ops_part_02.rs` (purge, `turn_state_*`, `token_usage`, `transcript_get`). Split for file-size, not module boundaries. RPC entry points return `RpcOutcome<ApiEnvelope<T>>` with request-id/count meta. |
