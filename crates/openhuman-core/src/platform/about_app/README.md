...
| `crates/openhuman-core/src/platform/about_app/catalog.rs` | Includes the `CAPABILITIES` data (`catalog_data.rs`) and holds the shared `CapabilityPrivacy` constants. Implements `all_capabilities`, `capabilities_by_category`, `lookup`, `search`, and the `ensure_validated` integrity check. |
