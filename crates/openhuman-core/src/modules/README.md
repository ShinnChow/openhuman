...
Cargo.toml comment beside `modules = ["tinybus/modules"]`). `documents`,
`web3`, and `voice` each turn a per-module host half on inside this directory
(`documents.rs`, `wallet.rs`, `voice.rs`) but do not gate the directory itself.

## Layout

| Path | Purpose |
| --- | --- |
| `mod.rs` | Crate-level rustdoc for the whole loading model; re-exports |
...
| `memory_host.rs` | Host-owned callbacks served *to* the TinyMemory module (embedding, chat compute, event publishing, Composio); sole survivor of the `host_impls` pair after the in-process engine left (openhuman#6161) |
...
5. The release cache downloads (if `modules.allow_download`), verifies the
   archive's SHA-256 against the digest pinned in `registry.rs`, extracts, and
   `dlopen`s — all off the module runtime's worker thread so a cold download
   never stalls another task.
6. tinybus's ABI descriptor, manifest, and digest gates decide whether the
   artifact is *admitted*; a faulted or refused module is cached as
   `LoadError::Failed` and never retried in this process — restart is the only
   recovery, because tinybus never unloads a library.

`boot::load_declared_modules` runs steps 3–6 at startup only for search-path
artifacts and `LoadPolicy::Eager` records (currently `tinymemory`, when the
memory driver actually selected the module-backed binding). Everything else is
...
- `crate::runtime::client` — the ungated facade over
  [`crate::modules::runtime`] for resolving/calling `tinyruntime`.
...
- `config::schema::modules::ModulesConfig` — `enabled`, `allow_download`,
  `install_dir`, and `overrides` control only whether the compiled-in modules
  in this directory load, never what is loadable.
