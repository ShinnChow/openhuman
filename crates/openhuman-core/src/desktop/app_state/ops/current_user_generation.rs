...
///
/// The freshness stamp goes with them. `clear_current_user_success` already
/// says why — "so the next account does not inherit this one's freshness"
/// — and sign-out is exactly the case it names; until now it ran only on
/// the deferred-rejection path (#5930).
///
...
pub(super) fn note_current_user_success_unless_stale(generation: u64, api_base: &str, token: &str) -> bool {
    ...
    note_current_user_success_locked(&mut success, api_base, token);
    true
}
...
fn publish_current_user_unless_stale(
    ...
    // answer 200 with no user, and the caller then falls back to `stored_user`,
    // so stamping here would report an age of ~0s for data that never changed.
    //
    // Taken before the cache lock and never inside it, because `ops_staleness`
    // requires that `LAST_CURRENT_USER_SUCCESS` never nest inside
    // `CURRENT_USER_CACHE`. Standing down here ends the publish: if sign-out has
    // landed, none of the three records may be written.
    if fetched.is_some() && !note_current_user_success_unless_stale(generation, api_base, token) {
        return false;
    }
