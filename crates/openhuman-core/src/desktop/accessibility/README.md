...
- `pub fn precompile_helper_background` — `helper.rs`, whose body is `include!`d from `helper_part_01.rs` (process management, `pub(crate) helper_send_receive`) and `helper_part_02.rs` (the embedded Swift source) — warm the Swift helper process in the background. No caller outside this module today.
...
- The Swift helper process compiled from the source embedded in `helper_part_02.rs` (stdin/stdout JSON), driven by `helper_send_receive`.
...
- `crates/openhuman-core/src/voice/always_on_part_02.rs` and `audio_capture.rs` — `detect_microphone_permission`, `microphone_denied_message`, `request_microphone_access`.
