...
- `pub fn precompile_helper_background` — `helper.rs`, whose body is `include!`d from `helper_part_01.rs` (process management, `pub(crate) helper_send_receive`) and `helper_part_02.rs` (the embedded Swift source) — compile the Swift helper with `swiftc` on a background thread so the first request avoids compile latency. No caller outside this module today.
