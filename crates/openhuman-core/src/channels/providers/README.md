...
- `bus::TelegramRemoteSubscriber` — busy-state handler for `DomainEvent::ChannelMessageReceived` / `ChannelMessageProcessed`, subscribed to the process bus in `channels/runtime/startup_part_01.rs`.
...
3. If the provider's secret lives outside `config.toml` (keyring or env), extend `hydrate_channel_credentials` in `runtime/startup_part_02.rs` the way `email` and `yuanbao` do; `tinychannels::build_channels` expects an already-hydrated config.
