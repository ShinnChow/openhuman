...
- `pub struct ElevatedOp` / `pub const ELEVATED_TOOLS` — `types.rs` — tools
  ...
  via `build_elevated_op` rather than silently bypassing the sandbox.
...
- **Docker** — `docker::docker_exec` runs `docker run --rm` with the host
  ...
  default, `--cap-drop ALL` (plus policy-specified extra drops), a
  read-only rootfs with `/tmp` and `/var/tmp` tmpfs mounts, memory/CPU
  limits, and only the explicit env passthrough list injected. Containers
  carry `openhuman.sandbox=true` for orphan cleanup.
...
backend has no usable OS jail and `create_sandbox_backend`/
`execute_local_jail` in `ops.rs` fall back to `cwd_jail::NoopBackend` (an
unenforced passthrough, reported as `SandboxStatus::Inactive` rather than
`Ready` so callers can tell the difference). Do not treat a resolved
`SandboxBackendKind` as a security boundary on its own.
...
- `crates/openhuman-core/src/agent/platform_shell.rs`,
  `agent/profiles/guard.rs` — reference this domain when documenting
  confinement guarantees for shell-spawning sites.
