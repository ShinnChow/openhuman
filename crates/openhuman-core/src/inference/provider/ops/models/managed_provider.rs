//! Managed-provider (`openhuman`) synthesis and session-safety helpers for
//! the `/models` listing.
//!
//! These exist because the MANAGED backend is not just another entry in
...
pub(super) fn synthesize_managed_entry(
    slug: &str,
    ...
    Some(CloudProviderCreds {
        ...
        endpoint: crate::config::schema::cloud_providers::CloudProviderType::Openhuman
            .default_endpoint()
            .to_string(),
...
/// than leaking it.
pub(super) fn url_is_credential_safe(url: &str) -> bool {
...
pub(super) fn managed_session_attaches(managed_token: &str, url: &str) -> bool {
...
pub(super) fn managed_401_means_signed_out(
    status: u16,
