...
| `schemas.rs` + `schemas_part_0{1,2}.rs`                                           | `inference.*` controller schemas + `handle_*` fns + param DTOs.                                                                                                                                                                                    |
...
| `local/ops.rs` + `local/ops_part_0{1,2}.rs`                                       | Local RPC entrypoints (`local_ai_status/prompt/summarize/vision_prompt/embed/should_react`, `ReactionDecision`); re-exported as `local::rpc`.                                                                                                      |
...
| `provider/factory.rs` + `factory_part_0{1..4}.rs`                                 | `create_chat_model*`, `provider_for_role`, provider-string grammar, access gates, and local/cloud/CLI model construction; `BYOK_INCOMPLETE_SENTINEL`.                                                                                              |
