...
| File | Role |
| --- | --- |
| `crates/openhuman-core/src/agent/artifacts/mod.rs` | Export-only: `mod` decls + re-exports of `ArtifactKind`/`ArtifactMeta`/`ArtifactStatus` and the controller-schema/registry pair. |
| `crates/openhuman-core/src/agent/artifacts/types.rs` | Serde domain types: `ArtifactKind` (presentation/document/image/other), `ArtifactStatus` (pending/ready/failed), `ArtifactMeta`. Each enum has `as_str`/`parse` (case-insensitive, fall back to `Other`/`Pending`). |
...
- `crate::config::Config` / `config::rpc` — source of `workspace_dir` and config loading in handlers.
