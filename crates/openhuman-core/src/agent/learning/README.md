...
| `schemas.rs` | `include!`s `schemas_part_01.rs` (controller schema list, `RegisteredController` table, the `linkedin_enrichment` / `save_profile` / `rebuild_cache` / `cache_stats` handlers, and the `get_cache()` helper) and `schemas_part_02.rs` (the facet handlers: `list_facets`, `get_facet`, `update_facet`, `pin_facet`, `unpin_facet`, `forget_facet`, `reset_cache`) for the `learning.*` namespace. |
...
| `linkedin_enrichment.rs` | Gmail→LinkedIn→Apify enrichment pipeline, `include!`-split into `linkedin_enrichment_part_01.rs` (`run_linkedin_enrichment`, `summarise_profile_with_llm`, `render_profile_markdown`, `scrape_linkedin_profile`) and `linkedin_enrichment_part_02.rs` (guard lookup + `put_profile_document` persistence). |
...
- `crate::inference::provider::create_chat_model_from_string_with_model_id` (profile summarisation) and `crate::inference::local::global` (local reflection route); `ReflectionHook` also accepts an optional `tinyinference::model::ChatModel` for the cloud fallback.
