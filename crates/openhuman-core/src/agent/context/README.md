...
- **`agent::harness`** — the primary consumer: `session/builder/builder_build.rs` constructs the `ContextManager`; `session/turn/core_turn.rs` drives the session-memory counters and spawns the archivist extraction when `should_extract_session_memory` says so; `session/turn/core_session_part_01.rs` reads the budget/compaction getters into the tinyagents turn config; `session/turn/session_io_impl_01_part_02.rs` reads `stats()`; `fork_context.rs`, `subagent_runner/*`, and `tool_filter.rs` consume prompt types.
- **`agent::tinyagents`** — `middleware_part_01.rs` and `mod_part_04.rs` read `CLEARED_PLACEHOLDER` when configuring `MicrocompactMiddleware`; `payload_summarizer.rs` and `host/context_composer.rs` build `PromptContext`s through `context::prompt`.
...
- **`channels`** — `channels/system_prompt.rs` (`ChannelSystemPrompt`, used by `channels/runtime/startup_part_01.rs`) calls `channels_prompt::build_system_prompt_with_identity`; `channels/mod.rs` re-exports `build_system_prompt`; the `channels/tests/*` prompt/identity tests call it directly.
