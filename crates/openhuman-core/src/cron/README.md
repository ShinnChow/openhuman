...
- `pub mod scheduler` (`pub async fn run(config: Config)`, `pub async fn execute_job_now` for the `cron.run` "Run Now" path) — `scheduler.rs`, split via `include!` into `scheduler_part_01.rs` (poll loop, `run`, `execute_job_now`, failure classifiers), `scheduler_part_02.rs` (`execute_job_with_retry`, per-job-type execution, `run_agent_job`, `build_agent_for_cron_job`, `run_flow_schedule_job`) and `scheduler_part_03.rs` (`deliver_if_configured`, `run_job_command_with_timeout`).
