...
- `crate::config` (`Config`, `HttpRequestConfig`, `schema::cloud_providers::{CloudProviderCreds, AuthStyle, CloudProviderType, generate_provider_id}`, `schema::{LocalAiConfig, LocalAiUsage, LlmBackend, ModelRouteConfig, SttEngine}`, `MODEL_CHAT_V1`, `MODEL_REASONING_QUICK_V1`) — the migrations read and mutate the config struct; `schema_version` is the gating field.
...
- `crate::inference::local::ollama_base_url_from_config` and `crate::inference::embeddings::factory` — the 6→7 step's Ollama reachability probe and the embedding-provider grammar it must stay in sync with.
- `crate::voice::factory` (`create_stt_provider`, `effective_stt_provider`) — the 9→10 step mirrors the STT slug resolution it repairs.
- `crate::agent::messages::ChatMessage` is imported only by tests.
