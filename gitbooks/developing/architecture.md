...
| **`Cargo.toml`** (root)     | Virtual workspace for `openhuman-core`, `openhuman-embed`, `openhuman-rpc`, and `openhuman-tui` (`cargo build --bin openhuman-core` builds the standalone CLI/server). `crates/openhuman-app/` is deliberately excluded and built with its own manifest; the desktop bundle links the core in-process rather than staging a sidecar binary. |
...
| **`docs/`**                 | Internal maintainer references not yet migrated to this book (coverage matrix, benchmarking, recipes)                                                                                                                                                                                                                                                                                                                               |

The desktop app **WebView** loads the UI from `app/`; heavy RPC and skills run in the **`openhuman-core`** core, hosted in-process as a tokio task by the Tauri shell and reachable over loopback HTTP (renderer → `coreRpcClient`, with the `relay_http_rpc` Tauri command as the host-side relay).
